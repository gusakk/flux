...
import (
	...
	"github.com/gusakk/flux/internal/pkg/syncutil"
...
func init() {
	...
	// when registering function with side effect it is significantly increase compilation time in complex queries
	// details are in MONITORING-283
	flux.RegisterPackageValue("http", "to", flux.FunctionValue(ToHTTPKind, createToHTTPOpSpec, toHTTPSignature))
...
func (t *ToHTTPTransformation) Process(id execute.DatasetID, tbl flux.Table) error {
	...
	if !ok {
		return errors.New(codes.Internal, "Could not get time column")
	}
				...
				for j, col := range er.Cols() {
					...
					case measurementNameCol != "" && measurementNameCol == col.Label:
						if col.Type != flux.TString {
							return errors.New(codes.Internal, "invalid type for measurement column")
						}
						m.name = er.Strings(j).ValueString(i)
					case isTag[j]:
						if col.Type != flux.TString {
							return errors.New(codes.Internal, "invalid type for tag column")
						}
